...
# Actual stan model run ---------------------------------------------------
    ...
    dens <- eventReactive(input$do_sim, {
        if(input$prior == "logRR prior"){
            ...
            lambda <- 1
            mu <- log(1-input$mu_ve) - input$sigma_ve^2 / 2
            sigma <- input$sigma_ve
        }
...
# Setting default priors by type ------------------------------------------
    observeEvent(input$prior, {
        if(input$prior == "logRR prior"){
            ...
            showTab(inputId = "options", target = "Basic Settings", select = TRUE)
            hideTab(inputId = "options", target = "Custom Settings")
        }
    })
    observeEvent(input$prior, {
        # disable("n")
        if(input$prior == "Very Skeptical"){
            updateNumericInput(session, "mu_ve", value = 0.0)
            updateNumericInput(session, "sigma_ve", value = 0.1)
            disable("mu_ve")
            disable("sigma_ve")
            
            # output$lambda <- renderUI(list(disabled(numericInput("p1", "p1", value = 1))))
            #  
            # output$mu <- renderUI(list(disabled(numericInput("mu1", "mu1", value = log(1-input$mu_ve) - input$sigma_ve^2/2))))
            # 
            # output$sigma <- renderUI(list(disabled(numericInput("sigma1", "sigma1", value = input$sigma_ve))))
            
        } else if(input$prior == "Skeptical"){
            updateNumericInput(session, "n", value = 1)
            updateNumericInput(session, "mu_ve", value = 0.1)
            updateNumericInput(session, "sigma_ve", value = 0.1)
            disable("mu_ve")
            disable("sigma_ve")
            ...
            updateNumericInput(session, "n", value = 1)
            updateNumericInput(session, "mu_ve", value = 0.3)
            updateNumericInput(session, "sigma_ve", value = 0.1)
            disable("mu_ve")
            disable("sigma_ve")
            ...
            updateNumericInput(session, "n", value = 1)
            updateNumericInput(session, "mu_ve", value = 0.5)
            updateNumericInput(session, "sigma_ve", value = 0.3)
            disable("mu_ve")
            disable("sigma_ve")
            # output$lambda <- renderUI(list(disabled(numericInput("p1", "p1", value = 1, min = 0, max = 1))))
            ...
            updateNumericInput(session, "n", value = 1)
            updateNumericInput(session, "mu_ve", value = 0.8)
            updateNumericInput(session, "sigma_ve", value = 0.3)
            disable("mu_ve")
            disable("sigma_ve")
            # output$lambda <- renderUI(list(disabled(numericInput("p1", "p1", value = 1, min = 0, max = 1))))
